...
public class bricks_smaller extends PApplet {
...
public void keyPressed(){
  if(gameStarted){
  //b.resetA();
  if(levelstarted && keyCode == LEFT){
    b.update(-1*bSpeed);

  }
  if(levelstarted && keyCode == RIGHT){
    b.update(bSpeed);
  }
  if(levelstarted && key == ' '){
...
class Bat{
 ...
 public void update(float veloc){
  v = veloc;
  a = v / 10;
 }
...
class Item{
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#FFFFFF", "bricks_smaller" });
  }
}
